...
- Eviction驱逐机制：
	...
	- ```
	  先看：idleTime > minEvictableIdleTimeMillis
	  再看：idleTime > softMinEvictableIdleTimeMillis AND idleCount > minIdle
	  ```
...
| `registerConnectionMBean` | `true` | 🔄可考虑关闭 | 是否为每个连接注册 JMX MBean                                  | [21](#0-20) [22](#0-21) |
...
| 参数                                    | 默认值     |       | 说明                                                                      |                         |
| ------------------------------------- | ------- | ----- | ----------------------------------------------------------------------- | ----------------------- |
| `accessToUnderlyingConnectionAllowed` | `false` | ✅保持关闭 | 是否允许通过 `DelegatingConnection.getInnermostDelegate()` 访问底层原始连接（有风险，谨慎使用） | [23](#0-22) [24](#0-23) |
